...
class Validation_helper {
	...
	public function set_rules( $key, $label, $rules ) {
		...
		foreach ( $tests_to_run as $test_to_run ) {
			...
			$this->run_validation_test( $validation_data, $rules, $key = '' );
		}
...
//    private function validate_file($key, $rules) {
	 ...
	 * @return void
	 */
	private function validate_file( array $validation_data, string $rules ) {
